...
uint8_t limits_get_state()
{
    ...
    for (idx = 0; idx < N_AXIS; idx++) {
        // MAX-Limits (falls vorhanden)
        pin = *max_limit_pins[idx] & (1 << max_limit_bits[idx]);
        if (!pin) { limit_state_max |= (1 << idx); } // Wenn 0V (Kontakt zu), dann Bit setzen

        // MIN-Limits (X=C2, Y=C3, Z=C4)
        pin = *min_limit_pins[idx] & (1 << min_limit_bits[idx]);
        if (!pin) { limit_state_min |= (1 << idx); } // Wenn 0V (Kontakt zu), dann Bit setzen
    }

    uint8_t limit_state = (limit_state_max | limit_state_min);

    // Invertierung ($5) sauber über XOR lösen
    if (bit_istrue(settings.flags, BITFLAG_INVERT_LIMIT_PINS)) {
        return (limit_state ^ 0b00000111);      // Dreht nur X, Y, Z um
    }
    return limit_state;
